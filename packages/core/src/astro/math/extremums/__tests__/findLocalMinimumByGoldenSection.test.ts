...
import { findLocalMinimumByGoldenSection } from "../findLocalMinimumByGoldenSection";

function polynomial(a: number, b: number, c: number, d: number) {
  return (x: number) => a * Math.pow(x, 3) + b * Math.pow(x, 2) + c * x + d;
}

describe("findLocalMinimumByGoldenSection", () => {
  it("should find local minimum", () => {

    const [result1, f_result1, rangeWidth1] = findLocalMinimumByGoldenSection(polynomial(0, 4, -6, 5), -2, 0, 2, { maxResultRangeWidth: 0.001, maxIterations: 25 });
    ...
    const [result2, f_result2, rangeWidth2] = findLocalMinimumByGoldenSection(polynomial(1, 4, 1, -2), -2, 0, 2);
