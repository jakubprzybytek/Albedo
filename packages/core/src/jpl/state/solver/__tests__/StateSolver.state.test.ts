...
describe("StateSolver", () => {
  ...
  it("should correctly compute uncorrected state for Earth wrt. Solar Barycenter", () => {
    const state = stateSolver.state(JplBodyId.Earth, JplBodyId.SolarSystemBarycenter, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.NONE);
    ...
    const reverseState = stateSolver.state(JplBodyId.SolarSystemBarycenter, JplBodyId.Earth, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.NONE);
  ...
  it("should correctly compute light time corrected state for Earth wrt. Solar Barycenter", () => {
    const state = stateSolver.state(JplBodyId.Earth, JplBodyId.SolarSystemBarycenter, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.LIGHT_TIME);
    ...
    const reverseState = stateSolver.state(JplBodyId.SolarSystemBarycenter, JplBodyId.Earth, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.LIGHT_TIME);
  ...
  it("should compute state for Moon wrt. Earth: uncorrected", () => {
    const state = stateSolver.state(JplBodyId.Moon, JplBodyId.Earth, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.NONE);
    ...
    const reverseState = stateSolver.state(JplBodyId.Earth, JplBodyId.Moon, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.NONE);
  ...
  it("should compute state for Moon wrt. Earth: light time corrected", () => {
    const state = stateSolver.state(JplBodyId.Moon, JplBodyId.Earth, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.LIGHT_TIME);
    ...
    const reverseState = stateSolver.state(JplBodyId.Earth, JplBodyId.Moon, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.LIGHT_TIME);
  ...
  it("should compute state for Venus wrt. Earth: uncorrected", () => {
    const state = stateSolver.state(JplBodyId.Venus, JplBodyId.Earth, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.NONE);
    ...
    const reverseState = stateSolver.state(JplBodyId.Earth, JplBodyId.Venus, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.NONE);
  ...
  it("should compute state for Venus wrt. Earth: light time corrected", () => {
    const state = stateSolver.state(JplBodyId.Venus, JplBodyId.Earth, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.LIGHT_TIME);
    ...
    const reverseState = stateSolver.state(JplBodyId.Earth, JplBodyId.Venus, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.LIGHT_TIME);
  ...
  it("should compute state for Venus wrt. Earth: converged light time corrected", () => {
    const state = stateSolver.state(JplBodyId.Venus, JplBodyId.Earth, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.CONVERGED_NEWTONIAN_LIGHT_TIME);
    ...
    const reverseState = stateSolver.state(JplBodyId.Earth, JplBodyId.Venus, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.CONVERGED_NEWTONIAN_LIGHT_TIME);
  ...
  it("should compute state for Venus wrt. Earth: light time and star aberration corrected", () => {
    const state = stateSolver.state(JplBodyId.Venus, JplBodyId.Earth, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.LIGHT_TIME_AND_STAR_ABBERATION);
    ...
    expect(state.position.z).approximately(-46416268.63226115, 1e-8);

    expect(state.velocity.x).approximately(35.22080413, 5e-3);
    expect(state.velocity.y).approximately(-45.92598905, 4e-3);
    expect(state.velocity.z).approximately(-21.93850053, 2e-3);

    expect(state.lightTime).approximately(819.26614366, 1e-8);

    const reverseState = stateSolver.state(JplBodyId.Earth, JplBodyId.Venus, EphemerisSeconds.fromDate(2019, 10, 9), CorrectionType.LIGHT_TIME_AND_STAR_ABBERATION);
    ...
    expect(reverseState.position.z).approximately(46413269.82387052, 5e-8);

    expect(reverseState.velocity.x).approximately(-35.22127253, 6e-3);
    expect(reverseState.velocity.y).approximately(45.92154901, 1e-2);
    expect(reverseState.velocity.z).approximately(21.93648380, 2e-3);

    expect(reverseState.lightTime).approximately(819.21320416, 1e-8);
  });
