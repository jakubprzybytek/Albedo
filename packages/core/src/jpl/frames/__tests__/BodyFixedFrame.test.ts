...
describe('BodyFixed', () => {
  ...
  describe('getRotationAngles', () => {
    it('should return rotation angles for Earth', () => {
      const bodyFixedFrame = frames.bodyFixedFrame(JplBodyId.Earth);
      ...
      approximately(bodyFixedFrame.getRotationAngles(es1), [-72.17289475, 0.11012664, 89.87326539], 1e-8);
      approximately(bodyFixedFrame.getRotationAngles(es2), [-71.18727125, 0.11014189, 89.87324784], 1e-8);
      approximately(bodyFixedFrame.getRotationAngles(es3), [-70.20164775, 0.11015714, 89.87323029], 1e-8);
    });

    it('should return rotation angles for Moon', () => {
      const bodyFixedFrame = frames.bodyFixedFrame(JplBodyId.Moon);

      const es1 = EphemerisSeconds.fromDate(2019, 10, 10);
      const es2 = EphemerisSeconds.fromDate(2019, 10, 11);
      const es3 = EphemerisSeconds.fromDate(2019, 10, 12);

      approximately(bodyFixedFrame.getRotationAngles(es1), [154.86483873, 23.80129410, -3.82071629], 1e-8);
      approximately(bodyFixedFrame.getRotationAngles(es2), [168.04679783, 23.80801176, -3.82564601], 1e-8);
      approximately(bodyFixedFrame.getRotationAngles(es3), [-178.77605679, 23.81506230, -3.82529824], 1e-8);
    });

    it('should return rotation angles for Mars', () => {
      const bodyFixedFrame = frames.bodyFixedFrame(JplBodyId.Mars);

      const es1 = EphemerisSeconds.fromDate(2019, 10, 10);
      const es2 = EphemerisSeconds.fromDate(2019, 10, 11);
      const es3 = EphemerisSeconds.fromDate(2019, 10, 12);

      approximately(bodyFixedFrame.getRotationAngles(es1), [103.08290492, 37.12572022, 47.65950925], 1e-8);
      approximately(bodyFixedFrame.getRotationAngles(es2), [93.97488994, 37.12572295, 47.65950463], 1e-8);
      approximately(bodyFixedFrame.getRotationAngles(es3), [84.86687495, 37.12572570, 47.65950004], 1e-8);
    });
  });
  ...
  describe('transformVector3', () => {
    ...
    it('should build transformation function for Earth', () => {
      ...
      const bodyFixedPositionVector = bodyFixedFrame.transformVector3(es)(position.toVector());

      expect(bodyFixedPositionVector[0]).approximately(-235289357.9726507, 4e-5);
      expect(bodyFixedPositionVector[1]).approximately(-48764919.67590383, 2e-4);
      expect(bodyFixedPositionVector[2]).approximately(-48699139.75889575, 1e-7);
    });
  });
