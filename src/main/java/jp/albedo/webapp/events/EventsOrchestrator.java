...
public class EventsOrchestrator {
    ...
    List<AstronomicalEvent> compute(Double fromDate, Double toDate, ObserverLocation observerLocation, RtsParameters rtsParameters, ConjunctionsParameters conjunctionsParameters) throws Exception {
        ...
        List<CatalogueType> conjunctionCatalogues = StreamUtils.collectPresent(
                Optional.ofNullable(conjunctionsParameters.isCataloguesDSEnabled() ? CatalogueType.NGC : null),
                Optional.ofNullable(conjunctionsParameters.isCataloguesDSEnabled() ? CatalogueType.IC : null));
